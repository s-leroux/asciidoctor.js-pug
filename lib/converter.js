...
/**
 * Load all templates files contained in the given directory
 *
 * The exact engine used to lead each file is determined by
 * the file extension.
 *
...
function load_templates(template_dirs) {
  ...
  template_dirs.forEach((template_dir) => {
    for(let filename of fs.readdirSync(template_dir)) {
      ...
      const engine = engines.engineFor(fullname);
      if (engine) {
