...
const asciidoctor = require('asciidoctor.js')() /* eslint-disable-line no-unused-vars */

const composite = require('./template-engines/composite.js')
...
(function () {
  ...
  const inherited = outils.patch(Document, '$create_converter', function () {
    ...
    */
    ...
    let template_engines = Opal.hash_delete(opts, 'template_engines')

    let engine = template_engines || composite.create()
    // If it has a forEach method, we assume this is an array-like object
    if (template_engines && template_engines.forEach) {
      engine = composite.create()
      template_engines.forEach((entry) => {
        engine.register(entry.patterns || '*', entry.engine)
      })
    }
    ...
    if (template_dirs) {
      ...
      template_dirs.forEach((template) => {
        templates = [engine.load(template_dirs)].concat(templates)
      })
    }
