...
const engine = require('./template-engines.js');
...
(function() {
  ...
  const inherited = outils.patch(Document, '$create_converter', function() {
    ...
    */
    ...
    let engines = template_engines || engine.create();
    ...
    if (template_engines && template_engines.forEach) {
      ...
      template_engines.forEach((entry) => {
        engines.register(entry.patterns || "*", entry.engine);
      });
    }
    ...
    if (template_dirs) {
      ...
      template_dirs.forEach((template) => {
        templates = [engines.load(template_dirs)].concat(templates);
      });
    }
